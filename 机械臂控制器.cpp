//机械臂控制器1.2 beta
/*1.当串口打开失败时，反复尝试直到成功打开
  2.添加打开串口成功的反馈信息
  3.串口变量全局化，利于后期加入多串口
  4.将串口初始化等程序移出main函数，提高主函数可读性和操作性*/
//by czl
//2019/2/21/21:40
...
//-------(￣▽￣)／程序开始---------
int main(){
	if (COM3() == FALSE) return 0;//接口初始化
	hello();//接收打招呼信息，准备完毕
	
	//开始对机械臂发出指令
	//WriteFile函数的使用参照上文ReadFile的注释
	//以下是示例：让0号舵机转到1700位置，再到1300位置，再到1500初始位置

	WriteFile(m_hComm, "{G0000#000P1700T1000!#001P1500T1000!#002P1500T1000!#003P1500T1000!#004P1500T1000!#005P1500T1000!}", 99, &dwactlen, NULL);
	PurgeComm(m_hComm, PURGE_TXCLEAR);//每次发完指令都要清空输出寄存器
	Sleep(2 * 1000);//延时2秒，延时是为了等待机械臂完成指令以后再发出下一个指令（中间还有电脑与机械臂传输数据，处理数据的时间），可根据实际情况调整
	WriteFile(m_hComm, "{G0001#000P1300T1000!#001P1500T1000!#002P1500T1000!#003P1500T1000!#004P1500T1000!#005P1500T1000!}", 99, &dwactlen, NULL);
	PurgeComm(m_hComm,PURGE_TXCLEAR);
	Sleep(2 * 1000);
	WriteFile(m_hComm, "{G0002#000P1500T1000!#001P1500T1000!#002P1500T1000!#003P1500T1000!#004P1500T1000!#005P1500T1000!}", 99, &dwactlen, NULL);
	PurgeComm(m_hComm,PURGE_TXCLEAR);

	
	printf("发送完毕！【若有指令出现遗漏，请检查延时和超时部分】\n");
